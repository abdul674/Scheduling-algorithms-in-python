...
def check_and_return_process_from_waiting_to_ready_queue(waiting, ready, current_time):
    temp_arr = []

    for i in range(len(waiting_queue)):
        if waiting[i]["return_time"] <= current_time:
            ready.put(waiting_queue[i])
            temp_arr.append(i)
    for i in temp_arr:
        waiting_queue.pop(i)
...
while process_completed < number_of_process:
    ...
    if process_queue.empty():
        ticks += 1
        waiting_queue, process_queue = check_and_return_process_from_waiting_to_ready_queue(waiting_queue,
                                                                                            process_queue,
                                                                                            ticks)
    ...
    if current_process["remaining_quanta"] is 0:
    ...
    ticks += min(current_process["remaining_quanta"], current_process["remaining_time"])
    current_process["remaining_time"] -= \
        min(current_process["remaining_time"], current_process["remaining_quanta"])
    current_process["remaining_quanta"] -= min(remaining_time, current_process["remaining_quanta"])
